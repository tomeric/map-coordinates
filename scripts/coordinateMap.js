class coord {
  ...
  addListener() {
    canvas.stage.addListener("click", (function (event) {
      if (!game.keyboard._downKeys.has(game.settings.get("map-coordinates", "keybind"))) return
      this.mouseCoords();
    }).bind(this))
  }
  ...
  constructor() {
    ...
    this.col0 = col0
    this.col1 = col1
    this.off = game.settings.get("map-coordinates", "offset")
    this.xValue = game.settings.get("map-coordinates", "xValue")
    this.yValue = game.settings.get("map-coordinates", "yValue")
    this.start = game.settings.get("map-coordinates", "startPoint")
    this.timeOut = game.settings.get("map-coordinates", "timeOut")
...
Hooks.on('canvasReady', () => {
  ...
  if (!canvas.MapCoordinates) {
    let map = new coord()
    canvas.MapCoordinates = map
  }

  window.MapCoordinates = canvas.MapCoordinates
});
